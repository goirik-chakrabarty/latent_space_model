...
session_path = "/mnt/vast-react/projects/neural_foundation_model/MOZAIK/mozaik_data_dynamic26872-17-20-Video-mozaik-trial_new1/"
session_name = "mozaik_data_dynamic26872-17-20-Video-mozaik-trial_new1"
...
if __name__ == "__main__":
    ...
    # Remove behavior modalities (not present in Mozaik synthetic data)
    del cfg["dataset"]["modality_config"]["eye_tracker"]
    del cfg["dataset"]["modality_config"]["treadmill"]
    cfg["dataset"]["out_keys"] = ["screen", "responses", "timestamps"]

    # Train dataloader: uses default valid_condition (tier="train")
    cfg["dataset"]["modality_config"]["responses"]["sampling_rate"] = 30
    cfg["dataset"]["modality_config"]["responses"]["chunk_size"] = 80
    # Override interpolation for spike data: SpikeInterpolator needs window/align
    # instead of SequenceInterpolator's interpolation_mode
    cfg["dataset"]["modality_config"]["responses"]["interpolation"] = {
        "interpolation_window": 1.0 / 30,  # bin width matching sampling rate
        "interpolation_align": "left",
    }
    # Disable normalization for spike responses (no meta/means.npy in this dataset)
    cfg["dataset"]["modality_config"]["responses"]["transforms"]["normalization"] = False
    # Disable nan_filter (requires SequenceInterpolator, incompatible with SpikeInterpolator)
    cfg["dataset"]["modality_config"]["responses"]["filters"] = {}
    ...
    # Upscale 36x64 screens to 144x256 to match original training protocol
    cfg["dataset"]["modality_config"]["screen"]["interpolation"]["rescale"] = True
    cfg["dataset"]["modality_config"]["screen"]["interpolation"]["rescale_size"] = [144, 256]
    ...
    train_dl = get_multisession_dataloader([session_path], cfg)
    ...
    print(f"n_neurons: {n_neurons_dict}")
    ...
    # Validation dataloader: use tier="validation"
    cfg["dataset"]["modality_config"]["responses"]["sampling_rate"] = 30
    ...
    dataloaders["oracle"] = {}
    session_key = session_name.split("dynamic")[-1].split("-Video")[0]
    dataloaders["oracle"][session_key] = get_multisession_dataloader(
        [session_path], cfg
    )
    ...
    checkpoint_dir = "../test_training/single_session_mozaik_26872_no_beh/"
    ...
    validation_score = standard_trainer(
        ...
        111,
        use_wandb=True,
        wandb_name="single_session_mozaik_26872_no_beh",
        loss_function="PoissonLoss",
        verbose=True,
        lr_decay_steps=4,
        lr_init=lr_inint,
